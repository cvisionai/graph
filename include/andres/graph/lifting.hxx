        ...
        if (cv[1] > 0)
        {
            ...
            for(std::size_t yPlus = cv[1]; yPlus > row0; --yPlus, ++offsetY)
            {
                ...
                for (std::size_t x = col0; x <= colN; ++x)
                {
                    ...
                    {
                        const std::size_t distance = (long)std::abs(static_cast<long>(x - cv[0]))
                            + (long)std::abs(static_cast<long>(yPlus - 1 - cv[1]));
        ...
        if (cv[1] < inputGraph.shape(1) - 1)
        {
            ...
            for (std::size_t y = row0; y <= rowN; ++y, ++offsetY)
            {
                ...
                for (std::size_t x = col0; x <= colN; ++x)
                {
                    if (metric == LiftingMetric::PathLength)
                    {
                        const std::size_t distance = (long)std::abs(static_cast<long>(x - cv[0])) + (long)std::abs(static_cast<long>(y - cv[1]));
